...
def test_with_ttp_generators():
    ...
    manual_commitment = add(multiply(g, randomness), multiply(h, attr_msk))
    manual_commitment = add(manual_commitment, multiply(hs[1], attr_value))
    ...
    resp_msk = responses[0][0]  # First response (msk)
    resp_attr = responses[0][1]  # Second response (attribute)
    resp_rand = responses[0][2]  # Third response (randomness)
    ...
    manual_verify = add(add(multiply(g, resp_rand), multiply(h, resp_msk)), multiply(add(commitment, neg(multiply(hs[1], attr_value))), challenge))
    
    # To check if this is correct, we need to know what the original witness was
    # Let's generate the same witness manually
    random.seed(12345)
    w_msk = random.randint(2, o)
    w_attr = random.randint(2, o)
    w_rand = random.randint(2, o)

    print(w_msk,w_attr, w_rand)
    
    manual_witness = add(multiply(g, w_rand), multiply(h, w_msk))
